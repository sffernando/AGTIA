...
//  LiquidLoader
//
//  Created by fernando on 2017/2/27.
...
//    var circleInter: CGFloat!
    ...
    override func setupShape() -> [LiquittableCircle] {
        return Array(0..<numberOfCircles).map{ i in
            return LiquittableCircle(center: CGPoint(x: self.circleInter + self.circleRadius + CGFloat(i) * (self.circleInter + 2 * self.circleRadius), y: self.loader.frame.height * 0.5), radius: self.circleRadius, color: self.color, growColor: self.growColor)
        }
    }
